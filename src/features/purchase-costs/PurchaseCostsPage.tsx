...
export function PurchaseCostsPage() {
  ...
  const hasUnconfirmedBudgets =
    result.status === "unavailable" &&
    result.reason === "POST_PURCHASE_BUDGET_NOT_CONFIRMED";
  ...
  const renderBudgetField = (
    field: "renovationBudget" | "movingSetupCosts",
    labelKey: string,
  ) => {
    const budget =
      field === "renovationBudget" ? renovationBudget : movingSetupCosts;
        ...
        <label className="budget-field__label">
              ...
              <div className="budget-field__input-group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    const digits = e.target.value.replace(/[^\d]/g, "");
                    const cents = digits
                      ? Math.round(parseFloat(digits) * 100)
                      : 0;
          ...
          <div id={`${field}-help`} className="budget-field__status">
            <select
              ...
              onChange={(e) =>
                setBudgetConfirmed(field, e.target.value as BudgetStatus)
              }
              className="budget-field__status-select"
            >
              <option value="not-budgeted">
                {t("purchase.budgetStatus.notBudgeted")}
              </option>
              <option value="confirmed-zero">
                {t("purchase.budgetStatus.confirmedZero")}
              </option>
            ...
            <Controller
              ...
              render={({ field }) => (
                <select
                  ...
                  aria-describedby={
                    errors.stateId ? "stateId-error" : undefined
                  }
                >
          ...
          <div className="advanced-fields">
            ...
            <div className="form-field">
              <label htmlFor="landRegisterRate">
                {t("purchase.results.landRegisterCosts")} (
                {t("purchase.rateLabel")})
              </label>
              <Controller
            ...
            {renderBudgetField(
              "renovationBudget",
              "budgetFields.renovationBudget",
            )}
            {renderBudgetField(
              "movingSetupCosts",
              "budgetFields.movingSetupCosts",
            )}
                ...
                render={({ field }) => (
                  <div className="form-field__input-group">
                    <input
                      ...
                      onChange={(e) =>
                        handleRateChange(e, "financedAcquisitionCostShare")
                      }
                      placeholder="0,00"
                      className="rate-input"
                    />
        ...
        <section className="results-section" aria-live="polite">
          ...
          {!hasEnteredPurchasePrice && (
            <p className="result-detail">
              {t("purchase.results.enterPurchasePrice")}
            </p>
          )}
          ...
          {hasErrors && (
            <div className="result-card error" role="alert">
              ...
              <p className="error-detail">
                {t("purchase.errors.field")}: {result.error?.field} |{" "}
                {t("purchase.errors.code")}: {result.error?.code}
              </p>
            </div>
          )}
            ...
            <>
              <div className="result-grid">
                <article className="result-card">
                  ...
                  <p className="result-detail">
                    ...
                    {(
                      availableResult.appliedAssumptions.transferTaxRate.value.toNumber?.() *
                        100 || 0
                    ).toFixed(2)}
                    %
                ...
                <article className="result-card">
                  ...
                  <p className="result-detail">
                    {t("purchase.results.rate")}:{" "}
                    {(
                      availableResult.appliedAssumptions.notaryRate.value.toNumber?.() *
                        100 || 0
                    ).toFixed(2)}
                    %
                    <span className="origin-badge">
                      {t(
                        `purchase.origin.${availableResult.appliedAssumptions.notaryRate.origin}`,
                      )}
                    </span>
                  </p>
                </article>
                ...
                {brokerInvolved && (
                  <article className="result-card">
                    ...
                    <p className="result-value">
                      {formatEuroInput(
                        availableResult.buyerBrokerCommissionCents,
                      )}{" "}
                      €
                    </p>
                    <p className="result-detail">
              ...
              <div className="result-card summary">
                ...
                <p className="result-value large">
                  {formatEuroInput(
                    availableResult.transactionAcquisitionCostsCents,
                  )}{" "}
                  €
                </p>
                <p className="result-detail">
                  {t("purchase.results.sumOfAbove")}
                </p>
              </div>
              ...
              <div className="result-card summary">
                ...
                <p className="result-detail">
                  {t("purchase.results.renovationPlusMoving")}
                </p>
              </div>
              ...
              <div className="result-card metadata">
                ...
                <dl>
                  ...
                  <dd>
                    {availableResult.appliedAssumptions.assumptionSetVersion}
                  </dd>
                  <dt>{t("purchase.results.transferTaxRateSourceDate")}</dt>
                  <dd>
                    {
                      availableResult.appliedAssumptions
